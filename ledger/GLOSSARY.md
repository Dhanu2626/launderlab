...
- **Blocking obligation** — a sanctions match must be acted on *before* the payment moves: freeze, reject, report. Unlike transaction monitoring, which is a review-when-you-reach-it duty, this has its own clock. It is why screening belongs in a separate queue from behavioural alerts — under one shared alert budget, sanctions hits score lowest and get cut first (measured in slice 7.10).
